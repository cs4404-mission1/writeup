...
**V-WEB-01**: Web Server sequential authorizaiton token generation
...
### Mitigating V-WEB-01: Web Server sequential authorizaiton token generation

The authorization cookie value generated by the webserver is an integer that is incrimented by 1 for each request. This was done out of convenience due to memory management. Due to Rust's concept of variable mutability in combination with the fact that Rocket has 16 responce-handling threads, an atomic value was used. This allowed the server to incriment a value on read without dealing with mutable borrows across threads. This value was stored in memory in the first space to prevent duplicate cookies. 

However, due to the short period of time that each cookie is valid, a securely random-generated unsigned 32-bit integer is enough to make the possibility of cookie replay and duplicate attacks extremely unlikley to succeed.
